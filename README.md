...
|------|-------------|----------|----------|----------|
| 哈希查找 | ~106ns | 最快 | 156MB | 高频查询，内存充足 |
| 二分查找 | ~255ns | 基准 | 6.2MB | 内存受限环境 |
| SIMD优化 | ~255ns | 相当 | 6.2MB | 批量查询优化 |
| 布隆过滤器 | ~180ns | 较快 | 6.5MB | 失败查询较多 |
...
|------|--------|------------|-----|----------|
| 哈希查找 | 103.2µs | 103ns | ~9,708,743 | +155% |
| SIMD查找 | 219.01µs | 219ns | ~4,566,210 | +20% |
| 布隆查找 | 253.67µs | 254ns | ~3,943,000 | +4% |
| 二分查找 | 263.3µs | 263ns | ~3,800,228 | 基准 |

### 初始化时间对比

| 算法 | 初始化时间 | 原因 | 冷启动影响 |
|------|------------|------|------------|
| 二分查找 | 2.2ms | 直接加载数据 | 最小 |
| SIMD查找 | 2.22ms | 与二分查找相同 | 最小 |
| 布隆查找 | 26.17ms | 额外构建布隆过滤器 | 小 |
| 哈希查找 | 136.2ms | 构建HashMap开销 | 显著 |
...
| 算法 | 查询时间 | 优势 | 适用场景 |
|------|----------|------|----------|
| 哈希查找 | 29-32ns | 直接判断key不存在 | 输入质量不高的场景 |
| 布隆过滤器 | ~175ns | 快速预过滤 | 大量无效查询场景 |
| 二分查找 | 44-175ns | 稳定可靠 | 通用场景 |
| SIMD查找 | ~175ns | 与二分查找相同 | 通用场景 |
